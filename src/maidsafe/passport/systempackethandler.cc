...
boost::shared_ptr<pki::Packet> SystemPacketHandler::GetPacket(
  ...
  if (it == packets_.end()) {
    ...
    boost::shared_ptr<pki::Packet> retrieved_packet;
    ...
    if (retrieved_packet) {
      ...
      if (packet_type == TMID || packet_type == STMID) {
        packet = boost::shared_ptr<TmidPacket>(new TmidPacket(
            *boost::shared_static_cast<TmidPacket>(retrieved_packet)));
      } else if (packet_type == MID || packet_type == SMID) {
        packet = boost::shared_ptr<MidPacket>(new MidPacket(
            *boost::shared_static_cast<MidPacket>(retrieved_packet)));
      } else if (IsSignature(packet_type, false)) {
        packet = boost::shared_ptr<SignaturePacket>(new SignaturePacket(
            *boost::shared_static_cast<SignaturePacket>(retrieved_packet)));
      } else {
#ifdef DEBUG
...
std::string SystemPacketHandler::SerialiseKeyring(
  ...
  while (it != packets_.end()) {
    if (IsSignature((*it).first, false) && (*it).second.stored) {
      boost::shared_static_cast<SignaturePacket>((*it).second.stored)->
