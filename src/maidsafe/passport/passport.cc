...
int Passport::SetNewUserData(const std::string &password,
  ...
  boost::shared_ptr<MidPacket> retrieved_pending_mid(PendingMid());
  boost::shared_ptr<MidPacket> retrieved_pending_smid(PendingSmid());
...
int Passport::ConfirmNewUserData(boost::shared_ptr<MidPacket> mid,
                                 boost::shared_ptr<MidPacket> smid,
                                 boost::shared_ptr<TmidPacket> tmid) {
  if (!mid || !smid || !tmid)
    return kNullPointer;
  return ConfirmUserData(mid, smid, tmid, boost::shared_ptr<TmidPacket>());
}
...
int Passport::UpdateMasterData(
  ...
  boost::shared_ptr<MidPacket> retrieved_mid(Mid()), retrieved_smid(Smid());
  if (!retrieved_mid)
    return kNoMid;
  if (!retrieved_smid)
    return kNoSmid;
  ...
  *smid_old_value = retrieved_smid->value();
  ...
  boost::shared_ptr<TmidPacket> retrieved_tmid(Tmid());
  if (!retrieved_tmid)
    return kNoTmid;
  boost::shared_ptr<TmidPacket> retrieved_stmid(Stmid());
  boost::shared_ptr<TmidPacket> retrieved_pending_stmid(PendingStmid());
  ...
  *new_tmid = *tmid;
  if (retrieved_stmid && (!(retrieved_pending_stmid &&
    ...
    *tmid_for_deletion = *retrieved_stmid;
    ...
    boost::shared_ptr<TmidPacket> empty_tmid(new TmidPacket);
    *tmid_for_deletion = *empty_tmid;
  }
  return kSuccess;
}

int Passport::ConfirmMasterDataUpdate(boost::shared_ptr<MidPacket> mid,
                                      boost::shared_ptr<MidPacket> smid,
                                      boost::shared_ptr<TmidPacket> tmid) {
  if (!mid || !smid || !tmid)
    return kNullPointer;
...
int Passport::ChangeUserData(
  ...
  if (!mid_for_deletion || !smid_for_deletion || !tmid_for_deletion ||
    ...
    return kNullPointer;
  boost::shared_ptr<MidPacket> retrieved_mid(Mid());
  if (!retrieved_mid)
    return kNoMid;
  boost::shared_ptr<MidPacket> retrieved_smid(Smid());
  if (!retrieved_smid)
    return kNoSmid;
  boost::shared_ptr<TmidPacket> retrieved_tmid(Tmid());
  if (!retrieved_tmid)
...
int Passport::ConfirmUserDataChange(boost::shared_ptr<MidPacket> mid,
                                    boost::shared_ptr<MidPacket> smid,
                                    boost::shared_ptr<TmidPacket> tmid,
                                    boost::shared_ptr<TmidPacket> stmid) {
...
int Passport::ConfirmPasswordChange(boost::shared_ptr<TmidPacket> tmid,
                                    boost::shared_ptr<TmidPacket> stmid) {
  if (!tmid || !stmid)
    return kNullPointer;
  boost::shared_ptr<MidPacket>null_mid;
...
int Passport::DoInitialiseSignaturePacket(
  ...
  if (signer_type != UNKNOWN) {
    boost::shared_ptr<SignaturePacket> signer =
        boost::shared_static_cast<SignaturePacket>(GetPacket(signer_type, true));
    if (!signer)
...
int Passport::ConfirmUserData(boost::shared_ptr<MidPacket> mid,
                              boost::shared_ptr<MidPacket> smid,
                              boost::shared_ptr<TmidPacket> tmid,
                              boost::shared_ptr<TmidPacket> stmid) {
...
std::string Passport::SignaturePacketName(const PacketType &packet_type,
  ...
  if (!IsSignature(packet_type, false))
    return "";
  boost::shared_ptr<SignaturePacket> packet(
      boost::shared_static_cast<SignaturePacket>(GetPacket(packet_type,
                                                          confirmed)));
...
boost::shared_ptr<MidPacket> Passport::Mid() {
  return boost::shared_static_cast<MidPacket>(GetPacket(MID, true));
}

boost::shared_ptr<MidPacket> Passport::Smid() {
  return boost::shared_static_cast<MidPacket>(GetPacket(SMID, true));
}

boost::shared_ptr<TmidPacket> Passport::Tmid() {
  return boost::shared_static_cast<TmidPacket>(GetPacket(TMID, true));
}

boost::shared_ptr<TmidPacket> Passport::Stmid() {
  return boost::shared_static_cast<TmidPacket>(GetPacket(STMID, true));
}

boost::shared_ptr<MidPacket> Passport::PendingMid() {
  return boost::shared_static_cast<MidPacket>(
      packet_handler_.GetPacket(MID, false));
}

boost::shared_ptr<MidPacket> Passport::PendingSmid() {
  return boost::shared_static_cast<MidPacket>(
      packet_handler_.GetPacket(SMID, false));
}

boost::shared_ptr<TmidPacket> Passport::PendingTmid() {
  return boost::shared_static_cast<TmidPacket>(
      packet_handler_.GetPacket(TMID, false));
}

boost::shared_ptr<TmidPacket> Passport::PendingStmid() {
  return boost::shared_static_cast<TmidPacket>(
