...
Keyword Session::keyword() const {
  std::lock_guard<std::mutex> lock(user_details_mutex_);
  return user_details_.keyword;
}

Pin Session::pin() const {
  return user_details_.pin;
}

Password Session::password() const {
  std::lock_guard<std::mutex> lock(user_details_mutex_);
...
void Session::set_session_name() {
  NonEmptyString random(RandomAlphaNumericString(8));
  std::lock_guard<std::mutex> lock(user_details_mutex_);
  NonEmptyString to_hash(user_details_.pin.data + user_details_.keyword.data + random);
  user_details_.session_name = NonEmptyString(EncodeToHex(crypto::Hash<crypto::SHA1>(to_hash)));
}
...
NonEmptyString Session::SerialiseDataAtlas() {
  ...
  if (has_drive_data()) {
    LOG(kInfo) << "SerialiseDataAtlas - Has drive data.";
