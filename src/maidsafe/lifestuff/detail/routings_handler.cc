...
RoutingsHandler::RoutingObject::RoutingObject()
    : maid(),
      routing(),
      search_id(),
      newtwork_health(-1),
...
RoutingsHandler::RoutingObject::RoutingObject(const Maid& maid, const NonEmptyString& search_id)
    : maid(new Maid(maid)),
      routing(new Routing(maid)),
      search_id(search_id),
      newtwork_health(-1),
      mutex(),
      condition_variable(),
...
RoutingsHandler::RoutingsHandler(ClientNfs& client_nfs, Session& session)
...
bool RoutingsHandler::AddRoutingObject(const Maid& maid,
                                       const std::vector<EndPoint>& bootstrap_endpoints,
                                       const NonEmptyString& search_id,
                                       const RequestPublicKeyFunctor& public_key_functor) {
  std::shared_ptr<RoutingObject> routing_object(std::make_shared<RoutingObject>(maid, search_id));
  // Bootstrap endpoints to udp endpoints
  std::vector<UdpEndPoint> peer_endpoints(GetUdpEndpoints(bootstrap_endpoints));
  // Functors
  routing::Functors functors;
  Maid::name_type name(routing_object->maid->name());
  functors.message_received = [this, name] (const std::string& wrapped_message,
                                            const NodeId& group_claim,
                                            const bool& /*cache_lookup*/,
                                            const routing::ReplyFunctor& reply_functor) {
                                              OnRequestReceived(name,
...
std::vector<RoutingsHandler::UdpEndPoint>
      RoutingsHandler::GetUdpEndpoints(const std::vector<EndPoint>& bootstrap_endpoints) {
