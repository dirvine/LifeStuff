...
class RoutingsHandler {
 public:
  ...
  typedef std::unique_ptr<Maid> MaidPtr;
  typedef routing::RequestPublicKeyFunctor RequestPublicKeyFunctor;

  explicit RoutingsHandler(ClientNfs& client_nfs, Session& session);
  ~RoutingsHandler();

  bool AddRoutingObject(const Maid& maid,
 ...
 private:
  struct RoutingObject {
    RoutingObject();
    RoutingObject(const Maid& maid, const NonEmptyString& search_id);
  ...
  Session& session_;
  std::map<Identity, std::shared_ptr<RoutingObject>> routing_objects_;
  mutable std::mutex routing_objects_mutex_;
