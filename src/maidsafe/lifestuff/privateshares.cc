...
// Multi Index
int PrivateShareHandler::AddPrivateShare(
...
int PrivateShareHandler::DeletePrivateShare(const std::string &value,
                                            const int &field) {
  if (field < 0 || field > 1)
    return -2011;
...
int PrivateShareHandler::TouchShare(const std::string &value,
                                    const int &field) {
  if (field < 0 || field > 1)
    return -2024;
...
int PrivateShareHandler::GetShareInfo(const std::string &value,
                                      const int &field,
                                      PrivateShare *ps) {
  if (field < 0 || field > 1)
    return -2014;
...
int PrivateShareHandler::GetShareList(std::list<private_share> *ps_list,
...
int PrivateShareHandler::GetFullShareList(const SortingMode &sm,
                                          const ShareFilter &sf,
                                          std::list<PrivateShare> *ps_list) {
  ps_list->clear();
  std::list<private_share> share_list;
  GetShareList(&share_list, sm, sf);
  while (!share_list.empty()) {
    PrivateShare ps;
    GetShareInfo(share_list.front().msid_, 1, &ps);
...
int PrivateShareHandler::GetParticipantsList(
    const std::string &value,
    const int &field,
...
void PrivateShareHandler::ClearPrivateShares() {
  pss_.clear();
  psps_.clear();
}
