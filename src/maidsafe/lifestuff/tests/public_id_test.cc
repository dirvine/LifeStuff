...
class PublicIdTest : public testing::Test {
 ...
 protected:
  void SetUp() {
    ...
    for (int i(0); i != 10; ++i)
      threads_.create_thread(std::bind(&ba::io_service::run, &asio_service_));
...
TEST_F(PublicIdTest, FUNC_CreateInvalidId) {
  EXPECT_EQ(passport::kFailedToCreatePacket,
            public_id1_.CreatePublicId("", false));
  EXPECT_EQ(passport::kFailedToCreatePacket,
            public_id1_.CreatePublicId("", true));
...
TEST_F(PublicIdTest, FUNC_CreatePublicIdSociable) {
  ...
  Sleep(interval_);
  EXPECT_EQ(public_username2_, received_public_username_);
  EXPECT_EQ(kSuccess, session1_->contacts_handler()->GetContactInfo(
                          received_public_username_, &received_contact));
