 ...
 public:
  ...
  void DoShareTest(
    ...
    if (message.subject() == "leave_share")
      return StopShareTest(sender, user_storage, message);
  ...
  void StopShareTest(
    ...
    ASSERT_EQ(kSuccess, user_storage->StopShare(sender, message.content(0)));
  }
 ...
 protected:
  ...
  std::shared_ptr<ClientController> CreateClientController(
    ...
    std::shared_ptr<ClientController> cc(
        new ClientController(asio_service_.service(), ss));
...
TEST_F(UserStorageTest, FUNC_CreateShare) {
  ...
  user_storage2_->MountDrive(*mount_dir_,
                             ...
                             session2_, true);
  fs::path directory1(user_storage2_->g_mount_dir() / tail);
...
TEST_F(UserStorageTest, FUNC_AddUser) {
  ...
  users.insert(std::make_pair(pub_name2_, false));
  ASSERT_EQ(kSuccess,
            user_storage1_->AddShareUsers(pub_name1_,
                                          directory0.root_directory() /
