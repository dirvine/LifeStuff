...
testing::AssertionResult NetworkHelper::StartLocalNetwork(std::shared_ptr<fs::path> test_root,
  ...
  // Write local bootstrap file for all other vaults/clients to use
...
//    args += " --peer " + endpoints.front();
  ...
  // Kill pd-keys-helper to kill zero-state vaults
  // TODO(Fraser#5#): 2012-08-31 - Don't just terminate, send signal.
  bp::terminate(zero_state_child);
  auto exit_code = wait_for_exit(zero_state_child, error_code);

  // Remove 2 zero-state vaults from local bootstrap file
  std::string serialised_bootstrap_nodes;
  ...
  if (protobuf_bootstrap.bootstrap_contacts_size() < 3)
    return testing::AssertionFailure() << "New contacts missing from bootstrap file.";
  endpoints.clear();
  for (int i(2); i != protobuf_bootstrap.bootstrap_contacts_size(); ++i) {
  ...
  exit_code = wait_for_exit(zero_state_child, error_code);
  if (error_code || exit_code)
    return testing::AssertionFailure() << "Executing " << pd::kKeysHelperExecutable()
                                       << " -ls returned " << exit_code << ",  error_code: "
                                       << error_code.message();

  // Sleep to allow vaults to remove zero-state peers from their routing tables
...
testing::AssertionResult NetworkHelper::StopLocalNetwork() {
  ...
  for (auto& vault_process : vault_processes_) {
    ...
    size_t size(vault_process.second->tellg());
