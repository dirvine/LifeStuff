...
//                      chunkstore_)),
 ...
 protected:
  void SetUp() {
    kademlia::Contact empty_contact, contact;
...
//  ASSERT_EQ(kad::kZeroId, contact.node_id().String());
  ...
  // empty pmid
  contact_cache_.Init(kademlia::NodeId());
  contact_cache_.WaitForUpdate();
  ASSERT_FALSE(contact_cache_.active());

  // complete init, trigger update
  contact_cache_.Init(kademlia::NodeId(pmid_));
  contact_cache_.WaitForUpdate();
...
//                      mko_.get(), good_response_, _1))));
  ...
  // RPC fails
  contact_cache_.Init(kademlia::NodeId(pmid_));  // implicit update
  contact_cache_.WaitForUpdate();
  ASSERT_FALSE(contact_cache_.active());
  ASSERT_TRUE(contact_cache_.last_update_.is_neg_infinity());
